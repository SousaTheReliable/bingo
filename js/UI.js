class UI {
  ...
  updateVillager(villagerId, villagerName, random = false) {
    if (this.card.activeSlot && this.card.activeSlot !== 13) {
      ...
      icon.onload = () => {
        ...
        const textHeightPosition =
          this.card.slotData.rows[row] + this.card.slotData.slotHeight - 10;

        this.ctx.drawImage(
          icon,
          this.card.slotData.columns[column] + 20,
          this.card.slotData.rows[row] + 15,
          60,
          60
        );
        ...
        this.ctx.fillRect(
          this.card.slotData.columns[column] + 20,
          this.card.slotData.rows[row] + 75,
          60,
          20
        );
        ...
        if (!this.firstTextPlaced) {
          ...
          this.ctx.fillText(
            villagerName,
            columnCenter - this.ctx.measureText(villagerName).width / 2,
            textHeightPosition
          );
        }
      };
  ...
  toggleRandomVerificationElement() {
    if (this.card.random) {
      ...
      this.ctx.fillText(
        'Certified Randomly Generated Card',
        370,
        695
      );
    } else {
  ...
  setEventListeners() {
    // Get Elements
    let selectorsElements = document.querySelectorAll(
  ...
  renderPortraitSelectors() {
    ...
    for (const animalType in animalsObj) {
      ...
      animalsObj[animalType][animalType].map((animal) => {
        ...
        animalPortraitContainer.setAttribute(
          'data-name',
